...
class extends Component {
	...
	public function getImages(): LengthAwarePaginator
	{
     ...
     if (!Storage::exists($imagesPath)) {
    \Log::warning("Le répertoire n'existe pas : " . $imagesPath);
    return new LengthAwarePaginator([], 0, $this->perPage, $this->page);
}
		 $allFiles   = Storage::allFiles($imagesPath);
        //$allFiles = Storage::disk('public')->files($imagesPath);
//    dd($imagesPath, $allFiles);
//"public/photos/2024/Nov" // resources/views/livewire/admin/images/index.blade.php:49
//[] // resources/views/livewire/admin/images/index.blade.php:49
    ...
    private function getYears(): Collection
    {
        return $this->getDirectories('public/photos', function ($years) {
            $this->selectedYear = $years->first()['id'] ?? date('Y');
            return $years;
        });
    }
    ...
    private function getMonths($year): Collection
    {
        return $this->getDirectories("public/photos/{$year}", function ($months) {
            $this->selectedMonth = $months->first()['id'] ?? date('M');
            $this->getImages();
            return $months;
        });
    }
